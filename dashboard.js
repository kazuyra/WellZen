// dashboard.js — Mind/Body/Soul categories + per-feature filtering (localStorage)

const $  = (sel, scope=document) => scope.querySelector(sel);
const $$ = (sel, scope=document) => Array.from(scope.querySelectorAll(sel));

/* ================= Storage ================= */
const KEY = 'tw_dash_v3';
const d = JSON.parse(localStorage.getItem(KEY) || '{}');
const data = Object.assign({
  xp: 0,
  level: 1,
  streak: 0,
  lastOpen: null,
  // habits carry a category
  habits: [
    { id: 'water',  name: 'Drink water (16oz)', cat:'body', days:{}, archived:false },
    { id: 'move',   name: '10 min movement',    cat:'body', days:{}, archived:false },
    { id: 'read',   name: 'Read 5 min',         cat:'mind', days:{}, archived:false },
  ],
  // logs now carry a category, so we can filter by Mind/Body/Soul
  focus: [],   // {start, end, minutes, cat:'mind'}
  moods: [],   // {time, emoji, note, cat:'soul'}
  sleep: [],   // {date, hours, bed, wake, cat:'body'}
  badges: [],
  ui: { cat:'all', subcat:'all' } // current selected category + in-tab subfilter
}, d);

const save      = () => localStorage.setItem(KEY, JSON.stringify(data));
const todayKey  = () => new Date().toISOString().slice(0,10);

/* ================= XP / Level / Streak ================= */
const addXP = (n) => {
  data.xp += n;
  data.level = Math.max(1, Math.floor(1 + Math.sqrt(data.xp/100)));
  $('#xpTotal').textContent = data.xp;
  $('#levelNum').textContent = data.level;
  save();
};

const updateStreak = () => {
  const t = todayKey();
  if (!data.lastOpen) { data.lastOpen = t; data.streak = 1; }
  else {
    const last = new Date(data.lastOpen);
    const now  = new Date(t);
    const diff = Math.round((now - last)/(1000*60*60*24));
    if (diff === 1) data.streak += 1;
    if (diff > 1)   data.streak = 1;
    data.lastOpen = t;
  }
  $('#globalStreak').textContent = data.streak;
  save();
};

/* ================= Tabs ================= */
$$('.tab').forEach(btn=>{
  btn.addEventListener('click', () => {
    $$('.tab').forEach(b=>b.classList.remove('is-active'));
    $$('.tab-panel').forEach(p=>p.classList.remove('is-active'));
    btn.classList.add('is-active');
    const id = btn.getAttribute('data-tab');
    document.getElementById(id).classList.add('is-active');
  });
});

/* ================= Category → visible tabs map ================= */
const CAT_TABS = {
  all:  ['todayTab','habitsTab','focusTab','moodTab','sleepTab','rewardsTab'],
  mind: ['todayTab','habitsTab','focusTab','rewardsTab'],
  body: ['todayTab','habitsTab','sleepTab','rewardsTab'],
  soul: ['todayTab','habitsTab','moodTab','rewardsTab'],
};

function showTabsFor(cat){
  const allow = new Set(CAT_TABS[cat] || CAT_TABS.all);
  // buttons
  $$('.tabs .tab').forEach(btn=>{
    const target = btn.getAttribute('data-tab');
    btn.style.display = allow.has(target) ? '' : 'none';
  });
  // panels
  $$('.tab-panel').forEach(p=>{
    p.style.display = allow.has(p.id) ? '' : 'none';
  });

  // if current active tab is hidden by new cat, jump to Today
  const active = $('.tabs .tab.is-active');
  if (active && active.style.display === 'none') {
    $$('.tabs .tab').forEach(b=>b.classList.remove('is-active'));
    $('#todayTab').classList.add('is-active');
    $(`[data-tab="todayTab"]`).classList.add('is-active');
    $$('.tab-panel').forEach(p=>p.classList.remove('is-active'));
    $('#todayTab').classList.add('is-active');
  }
}

/* ================= Category filter (top) ================= */
function setCategory(cat){
  data.ui.cat = cat;
  save();

  // toggle pills
  $$('.cat-filter .tag').forEach(b=>{
    b.classList.toggle('is-active', b.dataset.cat===cat);
  });

  // show/hide Today cards by their data-cat-panel
  $$('[data-cat-panel]').forEach(card=>{
    const c = card.getAttribute('data-cat-panel');
    card.style.display = (cat==='all' || cat===c) ? '' : 'none';
  });

  // adjust which feature tabs are visible
  showTabsFor(cat);

  // sync subfilter inside Habits tab
  setSubCategory(cat);

  // re-render everything under the current filters
  renderHabits();
  renderFocusLog();
  renderMoodLog();
  renderSleepLog();
  updateWeekBars();
  updateTodayNote();
}

$$('.cat-filter .tag').forEach(b=>{
  b.addEventListener('click', ()=> setCategory(b.dataset.cat));
});

/* ================= Subfilter in Habits tab ================= */
function setSubCategory(cat){
  data.ui.subcat = cat;
  $$('#habitsTab .chip').forEach(c=>{
    c.classList.toggle('is-active', c.dataset.subcat===cat);
  });
}

$$('#habitsTab .chip').forEach(c=>{
  c.addEventListener('click', ()=>{
    setSubCategory(c.dataset.subcat);
    renderHabits(); updateWeekBars(); updateTodayNote();
  });
});

/* Helpers */
const currentHabits = () => {
  const cat = data.ui.subcat==='all' ? data.ui.cat : data.ui.subcat;
  if (cat==='all') return data.habits.filter(h=>!h.archived);
  return data.habits.filter(h=>!h.archived && h.cat===cat);
};

/* ================= Habits ================= */
const habitList   = $('#habitList');
const todayHabits = $('#todayHabits');
const today       = todayKey();

function renderHabits() {
  const list = currentHabits();

  // full list
  habitList.innerHTML = '';
  list.forEach(h=>{
    const li = document.createElement('li');
    li.className = 'habit';
    const doneCount = Object.values(h.days).filter(Boolean).length;
    li.innerHTML = `
      <div class="left">
        <input type="checkbox" ${h.days[today] ? 'checked':''} data-habit="${h.id}" />
        <span class="name">${h.name}</span>
        <span class="muted small">• ${h.cat}</span>
      </div>
      <div class="right">
        <span class="muted small">${doneCount} ✓ total</span>
        <button class="icon edit" title="Edit" data-edit="${h.id}">✏️</button>
        <button class="icon archive" title="Archive" data-arch="${h.id}">🗄️</button>
      </div>`;
    habitList.appendChild(li);
  });

  // today’s checklist respects top category
  todayHabits.innerHTML = '';
  const todays = (data.ui.cat==='all')
    ? data.habits.filter(h=>!h.archived)
    : data.habits.filter(h=>!h.archived && h.cat===data.ui.cat);

  todays.forEach(h=>{
    const li = document.createElement('li');
    li.innerHTML = `
      <label class="check">
        <input type="checkbox" ${h.days[today] ? 'checked':''} data-habit="${h.id}" />
        <span>${h.name}</span>
      </label>`;
    todayHabits.appendChild(li);
  });

  // toggles, edit, archive
  $$('input[type="checkbox"][data-habit"]').forEach(cb=>{
    cb.addEventListener('change', () => {
      const id = cb.getAttribute('data-habit');
      const h = data.habits.find(x=>x.id===id);
      h.days[today] = cb.checked;
      if (cb.checked) addXP(5);
      save();
      updateWeekBars();
      updateTodayNote();
      renderBadges();
    });
  });

  $$('[data-edit]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const id = btn.getAttribute('data-edit');
      const h = data.habits.find(x=>x.id===id);
      const name = prompt('Edit habit name:', h.name) ?? h.name;
      const cat  = prompt('Category (mind / body / soul):', h.cat) ?? h.cat;
      h.name = name.trim() || h.name;
      h.cat  = /^(mind|body|soul)$/.test(cat) ? cat : h.cat;
      save(); renderHabits();
    });
  });

  $$('[data-arch]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      const id = btn.getAttribute('data-arch');
      const h = data.habits.find(x=>x.id===id);
      h.archived = true;
      save();
      renderHabits();
      updateWeekBars();
    });
  });
}

$('#addHabitBtn').addEventListener('click', ()=>{
  const name = prompt('New habit name (tiny & clear):');
  if (!name) return;
  const cat = prompt('Category (mind / body / soul):', 'body');
  if (!/^(mind|body|soul)$/.test(cat||'')) return alert('Use mind, body, or soul.');
  const id = name.toLowerCase().replace(/[^a-z0-9]+/g,'-').slice(0,24) + '-' + Math.random().toString(36).slice(2,6);
  data.habits.push({id, name: name.trim(), cat:cat.trim(), days:{}, archived:false});
  save();
  renderHabits();
});

$('#quickAddHabit').addEventListener('click', ()=> $('#addHabitBtn').click());

function updateTodayNote(){
  const list = (data.ui.cat==='all')
    ? data.habits.filter(h=>!h.archived)
    : data.habits.filter(h=>!h.archived && h.cat===data.ui.cat);
  const total = list.length || 1;
  const done  = list.filter(h=>h.days[today]).length;
  const pct   = Math.round((done/total)*100);
  $('#todayCompleteNote').textContent = `${done}/${total} done • ${pct}%`;
}

function updateWeekBars(){
  const days = [...Array(7)].map((_,i)=>{
    const d = new Date(); d.setDate(d.getDate()- (6-i));
    return d.toISOString().slice(0,10);
  });
  const actives = currentHabits();
  const html = days.map(k=>{
    const total = actives.length || 1;
    const done  = actives.filter(h => h.days[k]).length;
    const pct   = Math.round((done/total)*100);
    return `<div class="bar"><div class="fill" style="height:${pct}%"></div><span class="lab">${k.slice(5).replace('-','/')}</span></div>`;
  }).join('');
  $('#weekBars').innerHTML = html;
}

/* ================= Focus (Mind) ================= */
let focusInterval=null, focusRemaining=25*60, focusTotal=25*60;
const timeEl = $('#focusTime');

function fmt(sec){
  const m = Math.floor(sec/60).toString().padStart(2,'0');
  const s = Math.floor(sec%60).toString().padStart(2,'0');
  return `${m}:${s}`;
}
function setFocusMins(min){
  focusRemaining = focusTotal = min*60;
  timeEl.textContent = fmt(focusRemaining);
  $$('.pill').forEach(p=>p.classList.remove('is-active'));
  $(`.pill[data-min="${min}"]`).classList.add('is-active');
}
setFocusMins(25);

$('.timer-controls [data-action="startFocus"]').addEventListener('click', ()=>{
  if (focusInterval) return;
  focusInterval = setInterval(()=>{
    focusRemaining -= 1;
    timeEl.textContent = fmt(Math.max(0,focusRemaining));
    if (focusRemaining<=0){
      clearInterval(focusInterval); focusInterval=null;
      data.focus.push({start: Date.now()-focusTotal*1000, end: Date.now(), minutes: Math.round(focusTotal/60), cat:'mind'});
      addXP(10);
      save();
      renderFocusLog();
      alert('Focus complete! +10 XP');
    }
  }, 1000);
});
$('.timer-controls [data-action="pauseFocus"]').addEventListener('click', ()=>{
  clearInterval(focusInterval); focusInterval=null;
});
$('.timer-controls [data-action="resetFocus"]').addEventListener('click', ()=>{
  clearInterval(focusInterval); focusInterval=null;
  setFocusMins(focusTotal/60);
});
$$('.pill').forEach(p=>p.addEventListener('click', ()=> setFocusMins(Number(p.dataset.min)) ));

function renderFocusLog(){
  const cat = data.ui.cat;
  const filtered = cat==='all' ? data.focus : data.focus.filter(f=>f.cat===cat);
  const ul = $('#focusLog');
  ul.innerHTML = filtered.slice(-10).reverse().map(f=>{
    const d = new Date(f.end).toLocaleString();
    return `<li>${d} — ${f.minutes} min</li>`;
  }).join('') || '<li class="muted small">No sessions yet.</li>';
}

/* ================= Breathing / Mood (Soul) ================= */
let selectedMood = null;
$$('.mood').forEach(b=>{
  b.addEventListener('click', ()=>{
    selectedMood = b.dataset.mood;
    $$('.mood').forEach(x=>x.classList.remove('is-active'));
    b.classList.add('is-active');
  });
});
$('#saveMood').addEventListener('click', ()=>{
  if (!selectedMood) { alert('Pick a mood emoji first 🙂'); return; }
  data.moods.push({ time: Date.now(), emoji: selectedMood, note: $('#moodNote').value.trim(), cat:'soul' });
  addXP(2);
  $('#moodNote').value = '';
  $$('.mood').forEach(x=>x.classList.remove('is-active'));
  selectedMood = null;
  save();
  renderMoodLog();
});
function renderMoodLog(){
  const cat = data.ui.cat;
  const filtered = cat==='all' ? data.moods : data.moods.filter(m=>m.cat===cat);
  const ul = $('#moodLog');
  ul.innerHTML = filtered.slice(-15).reverse().map(m=>{
    const d = new Date(m.time).toLocaleString();
    const note = m.note ? ` — <span class="muted">${m.note}</span>` : '';
    return `<li>${m.emoji} ${d}${note}</li>`;
  }).join('') || '<li class="muted small">No moods yet.</li>';
}

/* ================= Sleep (Body) ================= */
$('#saveSleep').addEventListener('click', ()=>{
  const hours = Number($('#sleepHours').value);
  const bed   = $('#sleepBed').value || null;
  const wake  = $('#sleepWake').value || null;
  if (!hours && hours !== 0) return alert('Enter hours slept.');
  data.sleep.push({date: today, hours, bed, wake, cat:'body'});
  addXP(3);
  save();
  renderSleepLog();
});
function renderSleepLog(){
  const cat = data.ui.cat;
  const filtered = cat==='all' ? data.sleep : data.sleep.filter(s=>s.cat===cat);
  const ul = $('#sleepLog');
  ul.innerHTML = filtered.slice(-14).reverse().map(s=>{
    const hw = (s.bed && s.wake) ? ` • ${s.bed}–${s.wake}` : '';
    return `<li>${s.date}: ${s.hours} hrs${hw}</li>`;
  }).join('') || '<li class="muted small">No sleep logs yet.</li>';
}

/* ================= Badges ================= */
const BADGES = [
  { id:'first-check',   when: ()=> data.habits.some(h=>Object.values(h.days).some(Boolean)), label:'First Check ✓' },
  { id:'streak-3',      when: ()=> data.streak >= 3,  label:'3-day Streak' },
  { id:'streak-7',      when: ()=> data.streak >= 7,  label:'7-day Streak' },
  { id:'focus-3',       when: ()=> data.focus.length >= 3, label:'Focused x3' },
  { id:'sleep-7',       when: ()=> data.sleep.length >= 7, label:'Sleep Logger' },
  { id:'xp-200',        when: ()=> data.xp >= 200, label:'200 XP' },
];

function renderBadges(){
  BADGES.forEach(b=>{
    if (!data.badges.includes(b.id) && b.when()){
      data.badges.push(b.id); save();
    }
  });
  const grid = $('#badgeGrid');
  grid.innerHTML = BADGES.map(b=>{
    const earned = data.badges.includes(b.id);
    return `<div class="badge ${earned?'earned':''}" title="${b.label}">
      <span>${earned?'🏆':'🔒'}</span>
      <p>${b.label}</p>
    </div>`;
  }).join('');
}

/* ================= Init ================= */
function init(){
  $('#xpTotal').textContent = data.xp;
  $('#levelNum').textContent = data.level;
  updateStreak();

  // restore last category (default all)
  setCategory(data.ui.cat || 'all');

  renderHabits();
  renderFocusLog();
  renderMoodLog();
  renderSleepLog();
  updateWeekBars();
  updateTodayNote();
  renderBadges();
}
init();
